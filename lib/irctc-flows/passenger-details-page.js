...
class PassengerDetailsPage extends BasePage {
  ...
  async scrollToBottomAndHandleOptions(ticketData) {
    try {
      ...
      // Perform enough scrolls to ensure we reach the bottom
      for (let i = 0; i < 5; i++) {
      ...
      // SELECT PAYMENT METHOD - Fixed timeout issue
  ...
  async selectPaymentMethod(ticketData) {
    ...
    const selector = `//android.widget.TextView[@resource-id="cris.org.in.prs.ima:id/tv_payment_option" and contains(@text, "${modeText.substring(0, 20)}")]`;

    // Increased timeout to 15 seconds as per user feedback
    if (await this.safeClick(selector, 15000)) {
      return { success: true };
    }

    return { success: false, error: `Payment method "${modeText}" not found after 15 seconds` };
  }
}
